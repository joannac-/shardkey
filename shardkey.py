...
if __name__ == '__main__':
    ...
    shkey = raw_input("Which field do you want as your key? Just field name, please: ")
    ...
    mysort = []
    #query = "spec={}, fields={" + shkey + ":1}"
    idorder = raw_input("Do you want to insert in natural order? [y/n] (y) ")
    if idorder == "":
        idorder = "y"
    while not (idorder == "y" or idorder == "n"):
        print "Sorry, that's not a valid reply."
        idorder = raw_input("Do you want to insert in natural order? [y/n] (y) ")
        if idorder == "":
            idorder = "y"

    if idorder == "y":
        mysort = [("_id", pymongo.DESCENDING)]

        
    output = open("shard.log", "w")
    
    for doc in tcollout.find({}, fields={shkey:True}, sort=mysort):
        #print doc
